...
def handle_calculate_IK(req):
    ...
    if len(req.poses) < 1:
        ...
        for x in xrange(0, len(req.poses)):
            ...
            # Joint angle symbols
            theta1, theta2, theta3, theta4, theta5, theta6 = symbols('theta0:7')
            ...
            T6_G = Matrix([[             cos(qG),            -sin(qG),            0,              aG],
                           [ sin(qG)*cos(alphaG), cos(q7)*cos(alphaG), -sin(alphaG), -sin(alphaG)*dG],
                           [ sin(qG)*sin(alphaG), cos(q7)*sin(alphaG),  cos(alphaG),  cos(alphaG)*dG],
                           [                   0,                   0,            0,               1]])
	    ...
	    # roll, pitch, yaw = end-effector orientation
            ...
            wx = px - d5G * T_total[2,0]
            wy = py - d5G * T_total[2,1] 
            wz = pz - d5G * T_total[2,2]

            # Calculate Position, q1, q2, q3
            # ELBOW - DOWN
            height = wz - d01
            width = sqrt(power(wx, 2.0) + power(wy, 2.0)) - a12
            l = sqrt(power(height, 2.0) + power(width, 2.0))

            alpha = (height, width)
            beta = (power(a23, 2.0) + power(d34, 2.0) - power(l, 2.0)) / (2 * a23 * d34)
            gamma = (power(l, 2.0) + power(a23, 2.0) + power(d34, 2.0)) / (2 * a23 * l)
            ...
            if r13 == 0.0 and r23 == 0.0:
                ...
                # Non-Singular
                theta5 = atan2(sqrt(1-power(r33, 2.0)), r33) 
